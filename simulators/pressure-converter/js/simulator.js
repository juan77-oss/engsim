// simulator.js — Pressure Converter
...
function calculate() {
  ...
  if (rawValue === '') {
    showError('Please enter a pressure value.');
    return;
  }
  ...
  // This tool converts ABSOLUTE pressure, where 0 Pa represents a perfect
  // vacuum — the physical floor of the scale. Negative gauge-pressure
  // readings (e.g. from a vacuum gauge) are a different scale entirely;
  // see the educational content for that distinction.
  if (value < 0) {
    showError('Absolute pressure cannot be negative (0 represents a perfect vacuum).');
    return;
  }
  ...
  UNIT_ORDER.forEach((key) => {
    ...
    if (key === fromKey) row.classList.add('prc-row--active');
    ...
    tdValue.className = 'prc-value-cell';
