...
// Dynamic scientific-notation threshold (same fix as the other linear
// unit converters).
...
function calculate() {
  ...
  if (rawValue === '') {
    showError('Please enter a force value.');
    return;
  }
  ...
  // Force is a vector quantity — a negative value represents force in the
  // opposite direction (e.g. tension vs. compression), so it's allowed
  // here, consistent with Energy/Length/Mass/Volume/Area Converter.
  clearError();
  ...
  UNIT_ORDER.forEach((key) => {
    ...
    if (key === fromKey) row.classList.add('frc-row--active');
    ...
    tdValue.className = 'frc-value-cell';
