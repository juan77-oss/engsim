// simulator.js — Speed Converter
import { UNITS, UNIT_ORDER, DEFAULT_VALUE, DEFAULT_UNIT } from './constants.js';
import { convertAll, exceedsSpeedOfLight } from './core.js';
...
// --- Number formatting ---------------------------------------------------
// Dynamic scientific-notation threshold (same fix as the other linear unit
// converters): use Number(value.toFixed(decimals)) === 0 instead of a fixed
...
function calculate() {
  ...
  // Speed, as a scalar magnitude, has no meaningful negative value here
  // (similar in spirit to Data Storage Converter's floor at zero).
  if (value < 0) {
  ...
  // Physical ceiling: nothing can reach or exceed the speed of light in a
  // vacuum (same spirit as Temperature Converter's absolute-zero floor).
  if (exceedsSpeedOfLight(value, fromKey)) {
    showError('Speed cannot exceed the speed of light (c).');
    return;
  }
  ...
  UNIT_ORDER.forEach((key) => {
    ...
    if (key === fromKey) row.classList.add('spc-row--active');
    ...
    tdValue.className = 'spc-value-cell';
