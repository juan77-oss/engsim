// simulator.js — Data Storage Converter
...
function calculate() {
  ...
  if (rawValue === '') {
    showError('Please enter a data size value.');
    return;
  }
  ...
  // Unlike Energy/Length/Mass/Volume (which allow negatives for deltas),
  // a data size has no meaningful negative interpretation — a file can't
  // be -5 GB — so this is blocked here, similar in spirit to Temperature
  // Converter's absolute-zero floor.
  if (value < 0) {
    showError('Data size cannot be negative.');
    return;
  }
  ...
  UNIT_ORDER.forEach((key) => {
    ...
    if (key === fromKey) row.classList.add('dsc-row--active');
    ...
    tdValue.className = 'dsc-value-cell';
