// simulator.js — Flow Rate Converter
...
function calculate() {
  ...
  if (rawValue === '') {
    showError('Please enter a flow rate value.');
    return;
  }
  ...
  // A negative flow rate is meaningful (e.g. reverse/backflow, or net flow
  // out of a control volume), so it's allowed here, consistent with
  // Energy/Length/Mass/Volume/Area/Force/Torque Converter.
  clearError();
  ...
  UNIT_ORDER.forEach((key) => {
    ...
    if (key === fromKey) row.classList.add('flr-row--active');
    ...
    tdValue.className = 'flr-value-cell';
