...
// --- Number formatting ---------------------------------------------------
// Dynamic scientific-notation threshold (same fix as Power/Energy/Mass
...
// Global regex /\+/g (not a string literal) ensures ALL '+' characters are
// stripped — consistent fix applied across all linear-unit converters.
...
function calculate() {
  ...
  if (rawValue === '') {
    showError('Please enter an energy value.');
    return;
  }
  ...
  UNIT_ORDER.forEach((key) => {
    ...
    if (key === fromKey) row.classList.add('enc-row--active');
    ...
    tdValue.className = 'enc-value-cell';
